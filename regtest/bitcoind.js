...
var destKey = btccore.PrivateKey();
...
describe('Bitcoind Functionality', function() {
  ...
  before(function(done) {
    ...
    var regtestNetwork = btccore.Networks.get('regtest');
  ...
  describe('send transaction functionality', function() {
    ...
    it('will not error and return the transaction hash', function(done) {
      ...
      tx.sign(btccore.PrivateKey.fromWIF(utxos[0].privateKeyWIF));
    ...
    it('will emit "tx" events', function(done) {
      ...
      tx.sign(btccore.PrivateKey.fromWIF(utxos[2].privateKeyWIF));
