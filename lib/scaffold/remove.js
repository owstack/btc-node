...
/**
 * Will remove a service from btccore-node.json
...
/**
 * Will remove the Node.js service and from the btccore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The btccore-node configuration directory
...
function remove(options, done) {
  ...
  var btccoreConfigPath = path.resolve(configPath, 'btccore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(btccoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a btccore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to btccore-node.json
        removeConfig(btccoreConfigPath, service, next);
      });
    }, done
  );
}
