...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and btc-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/owstack/btc-node/blob/bitcoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed btc-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './btc-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in btc-node services directory
    ...
    var servicePackage = req('@owstack' + service.moduleName + '/package.json');
    var serviceModule = service.moduleName;

    // The package.json should not define both btcNode and owsNode
    if (servicePackage.btcNode) {
      serviceModule = service.moduleName + '/' + servicePackage.btcNode;
 ...
 * @param {Node} node - The Btc Node instance
 */
