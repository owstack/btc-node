...
function checkService(service) {
  if (!service.module.prototype ||
    ...
    throw new Error(
      ...
      (serviceInfo.moduleName != serviceInfo.name ? ' using module name ' + serviceInfo.moduleName : '') +
...
function lookInModuleManifest(req, service) {
  try {
    ...
    if (servicePackage.btccoreNode) {
      serviceModule = serviceModule + '/' + servicePackage.btccoreNode;
...
function loadModule(req, service) {
  ...
  //package.json file, if so, then see if there is a btccoreNode field, if so
  ...
  if (!serviceCode) {
    throw new Error('Attempted to load the ' + service.name + ' service from: ' +
      ...
      process.cwd() + '/package.json" - btccoreNode field. All paths failed to find valid nodeJS code.');
  }
