...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and btccore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/owstack/btccore-node/blob/bitcoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed btccore-node.json configuration file
...
function checkService(service) {
  ...
  if (!service.module.prototype ||
    ...
    throw new Error(
      'Could not load service ' + service.name + (service.moduleName != service.name ? ' using module name ' + service.moduleName : '') + ' as it does not support necessary methods and properties.'
    );
  }
}
...
function loadModule(req, service) {
  try {
    // first try in the built-in btccore-node services directory
    ...
    // The package.json should not define both btccoreNode and owsNode
    if (servicePackage.btccoreNode) {
      serviceModule = service.moduleName + '/' + servicePackage.btccoreNode;
 ...
 * @param {Node} node - The Btccore Node instance
 */
