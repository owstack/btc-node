...
function lookInCwd(req, service) {
  var location = service.config.cwdRequirePath ? service.config.cwdRequirePath : service.moduleName;
  try {
...
function lookInBuiltInPath(req, service) {
  try {
    var serviceFile = path.resolve(__dirname, '../services/' + service.moduleName);
    return req(serviceFile);
  } catch (e) {
    if (e.code !== "MODULE_NOT_FOUND") {
      log.error(e);
    }
    log.info('Checked the built-in path: lib/services, for service: ' + service.moduleName);
  }
}
...
function lookInModuleManifest(req, service) {
  try {
    ...
    if (servicePackage.owsNode) {
      serviceModule = serviceModule + '/' + servicePackage.owsNode;
      return req(serviceModule);
    }
  } catch(e) {
    log.info('Checked the module\'s package.json for service: ' + service.moduleName);
  }
}
...
function loadModule(req, service) {
  ...
  //package.json file, if so, then see if there is a owsNode field, if so
  ...
  if (!serviceCode) {
    throw new Error('Attempted to load the ' + service.moduleName + ' service from: ' +
      'the requirePath in the services\' config, then "' +
      process.cwd() + '" then from: "' + __dirname + '/../lib/services' + '" finally from: "' +
      process.cwd() + '/package.json" - owsNode field. All paths failed to find valid nodeJS code.');
  }
...
function setupServices(req, servicesPath, config) {
  ...
  if (config.services) {
    for (var i = 0; i < config.services.length; i++) {
      ...
      service.moduleName = config.servicesConfig[service.name].module || service.name;
