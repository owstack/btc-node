...
var BASE_PACKAGE = {
  description: 'A full Bitcoin node build with Btccore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    // 'btccore-lib': '^' + btccore.version,
    // 'btccore-node': version
    'btccore-lib': 'owstack/btccore-lib',
    'btccore-node': 'owstack/btccore-node'
  }
};
...
/**
 * Will create a base Btccore Node configuration directory and files.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/btccore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Btccore Node directory, configuration file,
 * bitcoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the btccore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the btccore-node directory and configuration
