...
var BASE_PACKAGE = {
  description: 'A full Bitcoin node build with Btc',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    '@owstack/btc-lib': '^' + btcLib.version,
    '@owstack/btc-node': '^' + version
  }
};
...
/**
 * Will create a base Btc Node configuration directory and files.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/btc-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Btc Node directory, configuration file,
 * bitcoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the btc node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the btc-node directory and configuration
