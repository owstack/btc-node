...
DB.prototype._setDataPath = function() {
  assert(this.node.datadir, 'Node is expected to have a "datadir" property');
  assert(fs.existsSync(this.node.datadir), 'Node "datadir" (' + this.node.datadir + ') does not exist');
  if (this.node.network === 'livenet' || this.node.network === 'mainnet') {
    this.dataPath = this.node.datadir + '/btccorenode.db';
  } else if (this.node.network === 'regtest') {
      this.dataPath = this.node.datadir + '/regtest/btccorenode.db';
  } else if (this.node.network === 'testnet') {
      this.dataPath = this.node.datadir + '/testnet/btccorenode.db';
  } else {
