...
Bitcoin.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  for(var i = 0; i < addresses.length; i++) {
    if (btccore.Address.isValid(addresses[i], this.node.network)) {
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of btccore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of btccore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of btccore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitcoind from btccore. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of btccore-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getDefaultConf = function() {
  ...
  if (this.node.network === btccore.Networks.testnet) {
...
Bitcoin.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = btccore.crypto.Hash.sha256sha256(message);
 ...
 * @param {String|Address|Array} addressArg - An address string, btccore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
...
Bitcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  ...
  function transformUnspentOutput(delta) {
    var script = btccore.Script.fromAddress(delta.address);
    return {
...
Bitcoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    ...
    if (clone) {
      // reverse start and end as the order in btccore is most recent to less recent
...
Bitcoin.prototype._getAddressStrings = function(addresses) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof btccore.Address) {
...
/**
 * Will retrieve a block as a Btccore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = btccore.Block.fromString(response.result);
...
/**
 * Will get a transaction as a Btccore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
