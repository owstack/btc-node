...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of btc-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of btc-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of btc-node. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of btc-node services will start.');
    node._reindex = true;
  }
};
...
/**
 * Will retrieve a block as a Btc object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
...
/**
 * Will get a transaction as a Btc Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
