...
Node.prototype._getServiceOrder = function(services) {
  ...
  function addToStack(moduleNames) {
    for(var i = 0; i < moduleNames.length; i++) {
      ...
      assert(service, 'Required dependency "' + moduleName + '" not available.');
...
Node.prototype._startService = function(serviceInfo, callback) {
  ...
  log.info('Starting ' + serviceInfo.moduleName + (serviceInfo.moduleName != serviceInfo.name ? ' as ' + serviceInfo.name : ''));
  ...
  if (serviceInfo.config) {
    ...
    assert(!serviceInfo.config.moduleName);
  ...
  async.eachSeries(
   ...
   function(service, next) {
     if (self.services[service.name]) {
      log.info('Stopping ' + service.moduleName + (service.moduleName != service.name ? ' running as ' + service.name : ''));
       self.services[service.name].stop(next);
     } else {
      log.info('Stopping ' + service.moduleName + (service.moduleName != service.name ? ' running as ' + service.name : '') + ' (not started)');
       setImmediate(next);
     }
   },
   function() {
